...
nf_pin=[] # store all lines that define nonfuel pin
with open(file,'r+') as f: # open initial file to get presets for optimizer

	for l in f:
		# locate fuel assembly map
		lis.append(l)
		if l.find('LPI')>=0 and l.find('INPUT')<0: # line preceeding pin lattice identified
			c=1
			continue
			...
			c=0
		# locate pin IDs
		if l.find('PIN')>=0 and l.find("FUEL")>=0: # fuel pin line identified
			f_pin.append(int(l.split()[1]))
		elif l.find('PIN')>=0 and l.find("'COO'")>=0: # nonfuel pin line identified
			nf_pin.append(int(l.split()[1]))
		else:
			continue
...
change_max=max(f_pin) # maximum fuel pin ID
mat_max=max(val) # maximum pin ID

# Acceptable list
Accept=[]
Standard=[] # no standard entry (all acceptable values based on initial core)

for i in range(1,change_max+1):
...
for i in range(len(val)):
	...
	if val[i] not in Standard:
		Accept.append([val[i]]) # special condition (must be in brackets)
	else:
		Accept.append(Standard)
...
def core_swap(Input): # Input - core-size integer array
	...
	if match(val,Accept)!=0: # if core input does not have values acceptable in all locations
		...
		my_file = open("Case_1_copy.inp", "w") # duplicate file
		...
		os.system("casmo4 Case_1_copy.inp")
		#print()

		# Step 5: Delete the unnecessary output files.
		os.system("rm Case_1_copy.cax")
		os.system("rm Case_1_copy.log")

		# Step 6: Read the output file and extract the optimized variable.
		file="Case_1_copy.out"	
...
problem = mlrose.DiscreteOpt(length = len(idx), fitness_fn = fitness_cust, maximize = True, max_val = mat_max) # range of 0-(max_ID-1) for array elements
