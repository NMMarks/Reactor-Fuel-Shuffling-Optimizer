...
# Separate fuel pins from nonlfuel pins
...
for i in range(1,mat_max+1):
	Standard.append(i) # all values from 1 to max fuel pin ID in map
"""
...
def core_swap(Input): # Input - core-size integer array
	...
	for i in range(0,len(Input)):
		#core_swap.append(str(Input[i])) #For fuel, minimal ID is 0
	...
	if match(val,Accept)!=0: # if core input does not have values acceptable in all locations
		...
		my_file = open(filename+"_copy.inp", "w") # duplicate file
		...
		os.system("casmo4 Case_2_copy.inp")
		#print()

		# Step 5: Delete the unnecessary output files.
		os.system("rm Case_2_copy.cax")
		os.system("rm Case_2_copy.log")

		# Step 6: Read the output file and extract the optimized variable.
		file="Case_2_copy.out"	
