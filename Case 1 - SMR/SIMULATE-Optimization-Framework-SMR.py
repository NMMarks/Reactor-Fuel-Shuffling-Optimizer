...
filename="Gen_SMR_Model_1" # naming convention for located original design and creating new designs
...
np.random.seed(1000) # RNG seed if desired; comment out if undesired.
...
N = 250 # sample limit of good runs
...
best_state_q, best_fitness_q, history_q = mlrose.simulated_annealing(problem_q, schedule = schedule_q,
															   max_attempts = 5, max_iters = 250,
															   init_state = init_state, curve=True, random_state = 15)
...
if L%2 == 1: # odd
	...
	best_state_c, best_fitness_c, history_c = mlrose.simulated_annealing(problem_c, schedule = schedule_c,
																   max_attempts = 5, max_iters = 50,
																   init_state = init_state, curve=True, random_state = 31)
...
# Enrichment Change
e_ID = [0,1,2] # element indexes to change in problem
...
best_state_e, best_fitness_e, history_e = mlrose.simulated_annealing(problem_e, schedule = schedule_e,
															   ...
															   init_state = r_init, curve=True, random_state = 74)
...
p_ID = [2] # element indexes to change in problem
...
best_state_p, best_fitness_p, history_p = mlrose.simulated_annealing(problem_p, schedule = schedule_p,
															   ...
															   init_state = s_init, curve=True, random_state = 89)
